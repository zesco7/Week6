...
/*UIImagePickerController
 ...
 3-2. : 취소 버튼 클릭시
 4. : 버튼액션을 통한 기능 구현(YPImagePickerButtonClicked, cameraButtonClicked, photoLIbraryButton 등)
...
class CameraViewController: UIViewController {
    ...
    @IBAction func clovaFaceButtonClicked(_ sender: UIButton) {
        ...
        //얼굴인식과정: 이미지뷰->네이버->얼굴분석요청->응답
        //문자열과 다르게 파일, 이미지, pdf 등은 파일 자체가 그대로 전송되지 않기 때문에 텍스트형태로 인코딩하여 전송해야 한다.(multipartFormData)
        //uiimage를 텍스트 형태로 변환해서 전달
        //서버는 전송되는 내용을 모르기 때문에 전송 파일 종류를 명시해줘야 한다. : Content-Type(콘텐트 타입은 라이브러리에 포함되어 있기 때문에 생략해도 됨)
    ...
    extension CameraViewController: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
        //UIImagePickerController3-1. : 뷰 기능 사용시(didFinishPickingMediaWithInfo: 사진을 선택 또는 카메라 촬영 직후 실행)
        ...
        //UIImagePickerController3-2. : 취소 버튼 클릭시
